...
export default class BookedRide extends Component {
  constructor(props){
    super(props)
    this.state = {
      leaveLocation: '',
  ...
  gettingUserRideDetails = async() =>{
    ...
    try {
      ...
      const leave_location = ride.pick_up_name
      const going_location = ride.drop_off_name
      const ride_price = ride.offered_ride_price
      const passenger_no = ride.offered_ride_passengers_no
      const ride_info = ride.offered_ride_info
      const car = ride.offered_user.car
      ...
      const pickedDate = new Date(ride.offered_ride_date_time)
      const user_name = ride.offered_user.name
      const user_phone_number = ride.offered_user.phone_number
  ...
  render() {
    return (
      ...
      {this.state.isloadingUser && <ActivityIndicator size="large" />}
        ...
        <TouchableWithoutFeedback onPress = {() => this.props.navigation.navigate('OfferedRideUserBio')}>
